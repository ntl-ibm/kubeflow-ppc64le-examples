...
class RendezvousTimeout:
    ...
    def __repr__(self):
        return f"{self._join} {self._last_call} {self._close} {self._heartbeat}"
...
@dataclass(repr=True, eq=False, frozen=True)
...
class _RendezvousState:
    ...
    def __repr__(self) -> str:
        part_str = ""
        for nd, i in self.participants.items():
            part_str += f"{repr(nd)} {i}\n"
        wait_str = [repr(w) for w in self.wait_list]
        return (
            f"round={self.round} complete={self.complete} deadline={self.deadline} closed={self.closed} \n"
...
def _remove_participant_epilogue(
    ...
    log.info(f"_remove_participants_epilogue {repr(state)}")
    if state.complete:
...
class _BackendRendezvousStateHolder(_RendezvousStateHolder):
    ...
    def sync(self) -> Optional[bool]:
        ...
        if self._dirty:
            ...
            if self._cache_duration > 0:
                ...
                if self._last_sync_time >= max(
                    ...
                    log.info("Sync Returning NONE to use cache....")
                    return None

            get_response = self._backend.get_state()
            log.info(f"Got state {'None' if not get_response else '<bytes>'}")
        ...
        if state_bits is not None:
            try:
                ...
                log.info(f"loaded state {repr(self._state)}")
            ...
            self._state = _RendezvousState()
            log.info(f"new state is {repr(self._state)}")
    ...
    def _sanitize(self) -> None:
        ...
        for dead_node in self._dead_nodes:
            log.info(
                f"Node {dead_node} is dead with heart beat at {state.last_heartbeats[dead_node]} expire time was {expire_time}"
            )
...
class _DistributedRendezvousOpExecutor(_RendezvousOpExecutor):
    ...
    def run(
        ...
        while action != _Action.FINISH:
            log.info(
                f"_DistributedRendezvousOpExecutor run() {repr(state_handler)} \n ACTION = {action}"
            )
...
def _should_keep_alive(ctx: _RendezvousContext) -> bool:
    ...
    try:
        ...
        log.info(f"_should_keep_alive did not get a heart beat for context {repr(ctx)}")
        return False
...
class _RendezvousJoinOp:
    ...
    def __call__(self, ctx: _RendezvousContext, deadline: float) -> _Action:
        state = ctx.state
        log.info(f"RendezvousJoinOp {repr(ctx)} {state}")
...
class _RendezvousCloseOp:
    ...
    def __call__(self, ctx: _RendezvousContext, deadline: float) -> _Action:
        log.info(f"RENDEZVOUS CLOSe op {repr(ctx)} {ctx.state}")
        if ctx.state.closed:
            return _Action.FINISH
        if time.monotonic() > deadline:
            return _Action.ERROR_TIMEOUT
...
class _RendezvousKeepAliveOp:
    ...
    def __call__(self, ctx: _RendezvousContext, deadline: float) -> _Action:
        log.info(f"RENDEZVOUS keep-alive op {repr(ctx)} {ctx.state}")
        if _should_keep_alive(ctx):
            if time.monotonic() > deadline:
                return _Action.ERROR_TIMEOUT
            return _Action.KEEP_ALIVE
...
class DynamicRendezvousHandler(RendezvousHandler):
    ...
    def from_backend(
        ...
        log.info(
            f"FROM BACKEND: {node} {repr(settings)} \n backend={type(backend)} \n {repr(backend)} cls {type(cls)} {repr(cls)}"
        )
