...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    print(
        f"(train) LOCAL_RANK {LOCAL_RANK} RANK {RANK} WORLD_SIZE {WORLD_SIZE} GIT_INFO {GIT_INFO} \n\n\n\n"
    )
    (
        save_dir,
        epochs,
...
if __name__ == "__main__":
    print(
        f"\n\n LOCAL_RANK {LOCAL_RANK} RANK {RANK} WORLD_SIZE {WORLD_SIZE} GIT_INFO {GIT_INFO} \n\n\n\n"
    )
    opt = parse_opt()
    main(opt)
