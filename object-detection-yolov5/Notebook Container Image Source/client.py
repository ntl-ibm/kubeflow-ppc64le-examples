...
class Client(object):
    ...
    def __init__(
        ...
        if srv_domain is not None:
            try:
                ...
                _log.error(
                    "Could not discover the etcd hosts from %s: %s", srv_domain, e
                )
    ...
    def machines(self):
        ...
        try:
            ...
            print(f">>>>>>>>>>>>> {uri}\n")
            print(f"{repr(self._get_headers())} <<<<<<<<<<<<<\n")
            ...
            machines = [
                node.strip()
                for node in self._handle_server_response(response)
                .data.decode("utf-8")
                .split(",")
            ]
    ...
    def members(self):
        ...
        self._members = {}
        try:
            data = self.api_execute(
                self.version_prefix + "/members", self._MGET
            ).data.decode("utf-8")
            res = json.loads(data)
    ...
    def leader(self):
        ...
        try:
            leader = json.loads(
                self.api_execute(
                    self.version_prefix + "/stats/self", self._MGET
                ).data.decode("utf-8")
            )
    ...
    def _stats(self, what="self"):
        ...
        data = self.api_execute(
            self.version_prefix + "/stats/" + what, self._MGET
        ).data.decode("utf-8")
        try:
    ...
    def write(self, key, value, ttl=None, dir=False, append=False, **kwdargs):
        ...
        """
        _log.debug(
            "Writing %s to key %s ttl=%s dir=%s append=%s", value, key, ttl, dir, append
        )
    ...
    def watch(self, key, index=None, timeout=None, recursive=None):
        ...
        if index:
            return self.read(
                key, wait=True, waitIndex=index, timeout=timeout, recursive=recursive
            )
        else:
    ...
    def eternal_watch(self, key, index=None, recursive=None):
        ...
        while True:
            response = self.watch(
                key, index=local_index, timeout=0, recursive=recursive
            )
    ...
    def _next_server(self, cause=None):
        ...
        _log.debug(
            "Selection next machine in cache. Available machines: %s",
            self._machines_cache,
        )
        try:
            ...
            raise etcd.EtcdConnectionFailed(
                "No more machines in the cluster", cause=cause
            )
        else:
    ...
    def _wrap_request(payload):
        ...
        def wrapper(self, path, method, params=None, timeout=None):
            ...
            while not response:
                ...
                try:
                    response = payload(
                        self, path, method, params=params, timeout=timeout
                    )
                    ...
                    if self._allow_reconnect:
                        _log.info(
                            "Reconnection allowed, looking for another " "server."
                        )
