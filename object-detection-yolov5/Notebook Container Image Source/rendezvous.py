...
def register_rendezvous_handler(scheme, handler):
    ...
    print(f"Register Rendezvous handler {scheme} - {handler}")
...
def _rendezvous_helper(url: str, rank: int, world_size_opt: Optional[int], **kwargs):
    result = urlparse(url)
    print(f"_rendezvous_helper result {repr(result)}")
    ...
    print(f"_rendezvous_helper #1 {rank} {world_size} {world_size_opt}")
...
def rendezvous(url: str, rank: int = -1, world_size: int = -1, **kwargs):
    ...
    print(f"rendezvous {url} {rank} {world_size}")
...
def _env_rendezvous_handler(
    ...
    print(f"_env_rendezvous_handler {url}")
    result = urlparse(url)
    query_dict = _query_to_dict(result.query)
