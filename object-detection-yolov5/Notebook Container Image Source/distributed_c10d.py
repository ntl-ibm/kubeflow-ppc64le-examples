...
for _red_op_name, _red_op_value in ReduceOp.RedOpType.__members__.items():
    setattr(
        ReduceOp,
        _red_op_name,
        _red_op_value if _red_op_name in DENY_LIST else ReduceOp(_red_op_value),
    )
...
def init_process_group(
    ...
    print(
        f"In init process group with world size {world_size} rank {rank} store {store}"
    )
    ...
    if backend == Backend.MPI:
        ...
        if store is None:
            print(
                f"Store is none, before rendezvous {world_size} rank {rank} store {store} rendez {rendezvous}"
            )
            rendezvous_iterator = rendezvous(
                init_method, rank, world_size, timeout=timeout
            )
            store, rank, world_size = next(rendezvous_iterator)
            print(f"Store was none, after next {world_size} rank {rank} store {store}")
...
def _new_process_group_helper(
    ...
    if backend == Backend.MPI:
        ...
        if backend == Backend.GLOO:
            ...
            print(
                f"ProcessGroupNCCL {prefix_store}, {group_rank}, {group_size}, {pg_options}"
            )
            ...
            assert (
                backend.upper() in Backend._plugins
...
def _check_for_nccl_backend(group):
    ...
    return is_nccl_available() and isinstance(pg, ProcessGroupNCCL)
